    ...
    <div className="py-2 px-4">
          ...
          {personalActivity.sport ? (
            <div className="flex gap-2">
              {personalActivity.sportAttributes?.map((sa: PersonalActivitySportAttribute) => {
                return (
          ...
          <div>日時: {dayjs(personalActivity.startDatetime).format('YYYY-MM-DD HH:mm')} - {personalActivity.endDatetime ? dayjs(personalActivity.endDatetime).format('YYYY-MM-dd HH:mm') :null}</div>
          ...
          {personalActivity.place ? (
            <h1 className="flex gap-2">
              ...
              {personalActivity.asGuest ? <div>
            <span>✅ ドロップイン</span></div> : null}

            {personalActivity.personalRecords?.map(pr => {
              return (
              <div className="flex flex-wrap items-center gap-2">
                <div>{pr.recordMaster.nameJa}</div>
                <div>{pr.recordValue} {pr.recordMaster.unitValue}</div>
                </div>
              )
            })}
